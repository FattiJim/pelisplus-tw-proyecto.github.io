<?php
    ...
    if ( $result = $con->query("SELECT * FROM serie WHERE eliminado = 0") ) {
        ...
        if(!is_null($rows)) {
            ...
            foreach($rows as $num => $row) {
                foreach($row as $key => $value) {
                    if($key == "idGenero"){
                        if($gen = $con->query("SELECT G.nombre as 'Genero' FROM serie S JOIN genero G ON S.idGenero = G.id AND  S.eliminado = 0")){
                        ...
                        if($reg = $con->query("SELECT R.descripcion as 'Region' FROM serie S JOIN region R ON S.idRegion = R.id AND  S.eliminado = 0")){
                        ...
                        if($clas = $con->query("SELECT C.clave, C.publico FROM serie S JOIN clasificacion C ON S.idClasificacion = C.id AND  S.eliminado = 0")){

<?php
    ...
    if(isset($_GET['id'])) {
        ...
        if ( $result = $con->query("SELECT p.id, p.imagen, p.usuario FROM perfil AS p JOIN cuenta AS c ON p.idCuenta = c.id JOIN usuario AS u ON
            u.idCuenta = c.id WHERE u.user = '$nombre' AND p.id = $idPerfil") ) {
